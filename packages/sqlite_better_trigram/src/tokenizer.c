// https://github.com/streetwriters/sqlite-better-trigram/blob/22412cae7dcf35db16d1521a2bd2c99216b84d67/tokenizer.c
// modifications:
// - changed uint16_t to uint32_t for u32 typedef
// - made implicit int casts explicit to avoid warnings
...
// changed this to actually be uint32_t instead of uint16_t
// idk why the original author used uint16_t here
...
static void tokenize(const char *pText, int nText, int foldCase,
  ...
  while (zIn < zEnd) {
    ...
    if (isSpace || cjk) {
      ...
      if (cjk) {
        ...
        int result =
            xToken(pCtx, 0, aBuf, (int)(zOut - aBuf), start, start + (int)(zOut - aBuf));
        if (result != 0)
          break;
      }
  ...
  if (i > 0) {
    xToken(pCtx, 0, aBuf, (int)(zOut - aBuf), aStart[0],
